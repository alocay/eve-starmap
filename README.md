...
Three layers are bundled today: `heatmapLayer` (per-system value visualization), `regionLabelLayer` (draws each region's name at the centroid of its member systems), and `routeLayer` (draws a jump route as a polyline, each leg colored by security status). Writing your own is just implementing the interface above -- see `packages/core/README.md#examples` for a full custom-layer example (a hover highlight ring) and the bundled layers' own examples.
...
- **Playground** (`playground/`) — interactive hands-on testing: real pan/zoom/click/hover, system search, heatmap/region-label toggles, and a route lookup (origin/destination + "Show route"). Serve from repo root (e.g. `npx serve .`) and open `/playground/index.html`.
